...
{
#define IN "null"
#define EXP CJSON_NULL
  char *buf = IN;
  FILE *stream = ecx_ccstreams_fstropen(&buf, "r");
  struct cjson *node = cjson_null_fscan(stream, NULL);

  fail_unless(node != NULL);
  {
    const char fmt[] = "Failed to scan null from stream. Got: %u Exp: 0x%2x";
    fail_unless(node->type == EXP, fmt, node->type, EXP);
  }
...
#define EXP "null"
  ...
  struct cjson *node = cjson_null_fscan(in_stream, NULL);
  fclose(in_stream);
  ...
  const char fmt[] = "Failed to print null to stream. Got: %s Exp: %s";
